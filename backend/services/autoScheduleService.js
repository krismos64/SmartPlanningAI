...
class AutoScheduleService {
  ...
  async generateWeeklySchedule(options) {
    try {
      // Valider les options
      if (!options.weekStart) throw new Error('Date de début de semaine requise');
      if (!options.departmentId) throw new Error('ID de département requis');
      if (!options.businessHours) throw new Error('Heures d'ouverture requises');
      ...
      if (!employees.length) throw new Error('Aucun employé actif trouvé dans ce département');
  ...
  async _getApprovedVacations(employeeIds, startDate, endDate) {
    try {
      ...
      const vacations = await db.query(
        ...
        [employeeIds, startDate, endDate, startDate, endDate, startDate, endDate]
      );
  ...
  async _getSourceWeekSchedules(departmentId, sourceWeek) {
    try {
      const sourceWeekStart = moment(sourceWeek).startOf('isoWeek').format('YYYY-MM-DD');
      ...
      console.error('Erreur lors de la récupération de la semaine source:', error);
      throw error;
    }
  }
  ...
  async _generateOptimizedSchedule(params) {
    ...
    if (sourceSchedules && sourceSchedules.length) {
      ...
      this._applySourceSchedule(scheduleData, sourceSchedules, employees, vacationDays);
    } else {
  ...
  _applySourceSchedule(scheduleData, sourceSchedules, employees, vacationDays) {
    ...
    sourceSchedules.forEach(source => {
      ...
      const sourceData = typeof source.schedule_data === 'string'
      ...
      days.forEach(day => {
        if (sourceData[day] && Array.isArray(sourceData[day])) {
          ...
          const dayDate = weekStartMoment.clone().add(dayIndex, 'days').format('YYYY-MM-DD');
          
          // Vérifier si l'employé est en congé ce jour-là
          const isOnVacation = vacationDays[employeeId] && 
  ...
  _generateScheduleFromScratch(
    ...
    days.forEach(day => {
      ...
      if (!businessHours[day] || !Array.isArray(businessHours[day]) || businessHours[day].length < 2) {
      ...
      slots.forEach(slot => {
        const availableEmployees = sortedEmployees.filter(emp => {
          ...
          const isOnVacation = vacationDays[emp.id] && 
                              vacationDays[emp.id].includes(dayDate);
        ...
        if (balanceRoles && Object.keys(roleMap).length > 1) {
          ...
          Object.entries(roleMap).forEach(([role, empIds]) => {
            const availableForRole = availableEmployees.filter(emp => 
              emp.role === role && !selectedEmployees.has(emp.id)
            );
          ...
          if (selectedEmployees.size < minimumEmployees) {
            const remaining = availableEmployees.filter(emp => 
              !selectedEmployees.has(emp.id)
            );
            
            for (let i = 0; i < minimumEmployees - selectedEmployees.size && i < remaining.length; i++) {
          ...
          const toSelect = Math.min(minimumEmployees, availableEmployees.length);
    ...
    while (improved && iteration < maxIterations) {
      ...
      for (const day of days) {
        ...
        for (let i = 0; i < employees.length; i++) {
          ...
          const isAOnVacation = vacationDays[empA.id] && 
                               vacationDays[empA.id].includes(dayDate);
          ...
          for (let j = i + 1; j < employees.length; j++) {
            ...
            const isBOnVacation = vacationDays[empB.id] && 
                                 vacationDays[empB.id].includes(dayDate);
            ...
            const hoursA = slotsA.reduce((sum, slot) => sum + (slot.end - slot.start), 0);
            const hoursB = slotsB.reduce((sum, slot) => sum + (slot.end - slot.start), 0);
        ...
        days.forEach(day => {
          ...
          preferredSlots.forEach(preferred => {
            ...
            const isMatched = assignedSlots.some(assigned => {
              return assigned.start <= preferred.start && assigned.end >= preferred.end;
            });
