...
const SidebarContainer = styled.aside`
  ...
  padding: 0 0 ${({ theme }) => theme.spacing.lg} 0; /* Padding seulement en bas */
  overflow-y: auto;
  z-index: 10;
  display: flex;
...
const SectionTitle = styled.h3`
  ...
  letter-spacing: 1px;
  margin: ${({ theme }) =>
    `${theme.spacing.lg} ${theme.spacing.xl} ${theme.spacing.sm}`};
  opacity: 0.7;
`;
...
const Sidebar = () => {
  ...
  // S'assurer que le composant est correctement monté
  useEffect(() => {
    setMounted(true);
    return () => {
      setMounted(false);
  ...
  const navLinks = [
    ...
    { to: "/employees", icon: <EmployeesIcon />, label: t("navbar.employees") },
    {
      to: "/weekly-schedule",
      icon: <PlanningIcon />,
      label: t("navbar.schedule"),
    },
    { to: "/vacations", icon: <VacationsIcon />, label: t("navbar.vacations") },
    { to: "/activities", icon: <ReportsIcon />, label: t("navbar.activities") },
    { to: "/stats", icon: <StatsIcon />, label: t("navbar.stats") },
